...
function requete(body: any) {
  return new Request("http://localhost/api/ai/actions/relance", {
    method: "POST",
...
describe("POST /api/ai/actions/relance", () => {
  it("retourne 400 si factureId est manquant", async () => {
    const res = await POST(requete({}));
    expect(res.status).toBe(400);
  });

  it("retourne 400 si factureId n est pas un nombre", async () => {
    const res = await POST(requete({ factureId: "abc" }));
    expect(res.status).toBe(400);
  });

  it("retourne l email et degraded en cas de succes", async () => {
    (genererEmailRelance as any).mockResolvedValue({ texte: "Email genere", degraded: false });

    const res = await POST(requete({ factureId: 1 }));
    const json = await res.json();

    expect(res.status).toBe(200);
    expect(json.email).toBe("Email genere");
  ...
  it("retourne 500 en cas d erreur inattendue", async () => {
    (genererEmailRelance as any).mockRejectedValue(new Error("Panne serveur"));
